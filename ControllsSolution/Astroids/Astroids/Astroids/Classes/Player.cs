...
namespace Astroids.Classes
{
    class Player
    {
        ...
        private ControlHandler cHandler;
        ...
        public void Update(GameTime gameTime)
        {
            ...
            if (cHandler.GetInput().Contains("Up"))
            {
                Move();
                playerTextureIdle = playerTextureMoving;
            }
            if(!cHandler.GetInput().Contains("Up")) ///misschien else gebruiken?
            {
                SlowDown();
                playerTextureIdle = defaultTexture;
            }
            if (cHandler.GetInput().Contains("Right"))
            {
                rotationAngle = rotationAngle + 0.1f;
                playerPos = playerPos + oldDirection;
                bulletDirection = new Vector2((float)Math.Cos(rotationAngle), (float)Math.Sin(rotationAngle));
            }
            if (cHandler.GetInput().Contains("Left"))
            {
                rotationAngle = rotationAngle - 0.1f;
                playerPos = playerPos + oldDirection;
                bulletDirection = new Vector2((float)Math.Cos(rotationAngle), (float)Math.Sin(rotationAngle));
            ...
            if (delay <= 0)
            {
                if (cHandler.GetInput().Contains("Shoot"))
                {
